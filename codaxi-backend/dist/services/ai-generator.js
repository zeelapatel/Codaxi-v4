...
exports.generateFromContextPack = generateFromContextPack;
/**
...
function dbg(...args) { if (LLM_DEBUG)
    console.log('[LLM][debug]', ...args); }
function clampText(s, max = LLM_LOG_CONTEXT_MAX) { return !s ? s : (s.length > max ? (s.slice(0, max) + `\n... <clipped ${s.length - max} chars>`) : s); }
...
No comments or markdown.`;
    ...
    try {
        ...
        if (LLM_LOG_FINAL_JSON)
            dbg('generate.withLLM.finalJson', clampText(JSON.stringify(out)));
        return out;
    }
    catch (e) {
        return generateApiExamples(opts);
...
function normalizeOutput(json) {
    ...
    if (Array.isArray(raw?.errors)) {
        errors = raw.errors.filter((e) => e && typeof e === 'object').map((e) => ({
    ...
    const out = {
        ...
        requestExample: raw.requestExample ? (0, sanitize_1.clampJsonSize)(raw.requestExample) : undefined,
        responses,
        errors
    };
    return out;
}
async function generateFromContextPack(pack) {
...
No comments or markdown. If unsure, leave fields empty rather than guessing.`;
    ...
    if (pack.endpoint.consumes)
        headerLines.push(`Consumes: ${pack.endpoint.consumes}`);
    if (pack.endpoint.produces)
        headerLines.push(`Produces: ${pack.endpoint.produces}`);
    if (pack.endpoint.auth)
        headerLines.push(`Auth: ${pack.endpoint.auth}`);
    if (pack.facts?.length)
        headerLines.push(`Facts: ${pack.facts.join('; ')}`);
    const sections = [];
    const byKind = (k) => pack.contexts.filter(c => c.kind === k);
    ...
    try {
        ...
        if (LLM_LOG_FULL_CONTEXT)
            dbg('generate.fromPack.fullUserContent', clampText(userContent));
    }
    catch { }
    const messages = [
    ...
    try {
        ...
        if (LLM_LOG_FINAL_JSON)
            dbg('generate.fromPack.finalJson', clampText(JSON.stringify(out)));
        return out;
    }
    catch (e) {
        dbg('generate.fromPack.error', { message: e?.message });
        return { params: undefined, requestSchema: undefined, requestExample: undefined, responses: { '200': { contentType: 'application/json', example: {} } }, errors: [{ status: 400, message: 'Unknown' }] };
    }
}
